...
FRONTEND_FORM_BUILDER = frontend/form-builder
FRONTEND_FORM_BUILDER_YARN_MODULES = $(FRONTEND_FORM_BUILDER)/node_modules
FRONTEND_FORM_BUILDER_DIST = $(FRONTEND_FORM_BUILDER)/dist
FRONTEND_FORM_BUILDER_DIST_FINAL = frontend/public/static/form-builder
FRONTEND_FORM_BUILDER_DEPS = \
	$(FRONTEND_FORM_BUILDER_YARN_MODULES) \
	$(FRONTEND_FORM_BUILDER)/package.json \
	$(FRONTEND_FORM_BUILDER)/tsconfig.json \
	$(FRONTEND_FORM_BUILDER)/vite.config.ts \
	$(shell find $(FRONTEND_FORM_BUILDER)/src -type f 2>/dev/null)

FRONTEND_FORM_EMBED = frontend/form-embed
FRONTEND_FORM_EMBED_YARN_MODULES = $(FRONTEND_FORM_EMBED)/node_modules
FRONTEND_FORM_EMBED_DIST = $(FRONTEND_FORM_EMBED)/dist
FRONTEND_FORM_EMBED_DIST_FINAL = static/public
FRONTEND_FORM_EMBED_DEPS = \
	$(FRONTEND_FORM_EMBED_YARN_MODULES) \
	$(FRONTEND_FORM_EMBED)/package.json \
	$(FRONTEND_FORM_EMBED)/tsconfig.json \
	$(FRONTEND_FORM_EMBED)/rollup.config.js \
	$(shell find $(FRONTEND_FORM_EMBED)/src -type f 2>/dev/null)
...
# Build the JS form-builder dist.
$(FRONTEND_FORM_BUILDER_YARN_MODULES): $(FRONTEND_FORM_BUILDER)/package.json
	cd $(FRONTEND_FORM_BUILDER) && $(YARN) install
	touch -c $(FRONTEND_FORM_BUILDER_YARN_MODULES)

$(FRONTEND_FORM_BUILDER_DIST): $(FRONTEND_FORM_BUILDER_DEPS)
	export VUE_APP_VERSION="${VERSION}" && cd $(FRONTEND_FORM_BUILDER) && $(YARN) build
	touch -c $(FRONTEND_FORM_BUILDER_DIST)

$(FRONTEND_FORM_BUILDER_DIST_FINAL): $(FRONTEND_FORM_BUILDER_DIST)
	mkdir -p $(FRONTEND_FORM_BUILDER_DIST_FINAL)
	cp -r $(FRONTEND_FORM_BUILDER_DIST)/* $(FRONTEND_FORM_BUILDER_DIST_FINAL)
	touch -c $(FRONTEND_FORM_BUILDER_DIST_FINAL)

# Build the JS form-embed dist.
$(FRONTEND_FORM_EMBED_YARN_MODULES): $(FRONTEND_FORM_EMBED)/package.json
	cd $(FRONTEND_FORM_EMBED) && $(YARN) install
	touch -c $(FRONTEND_FORM_EMBED_YARN_MODULES)

$(FRONTEND_FORM_EMBED_DIST): $(FRONTEND_FORM_EMBED_DEPS)
	cd $(FRONTEND_FORM_EMBED) && $(YARN) build
	touch -c $(FRONTEND_FORM_EMBED_DIST)

$(FRONTEND_FORM_EMBED_DIST_FINAL)/lm-forms.js: $(FRONTEND_FORM_EMBED_DIST)
	mkdir -p $(FRONTEND_FORM_EMBED_DIST_FINAL)
	cp $(FRONTEND_FORM_EMBED_DIST)/lm-forms.min.js $(FRONTEND_FORM_EMBED_DIST_FINAL)/lm-forms.js
	touch -c $(FRONTEND_FORM_EMBED_DIST_FINAL)/lm-forms.js

.PHONY: build-frontend
build-frontend: $(FRONTEND_EMAIL_BUILDER_DIST_FINAL) $(FRONTEND_FORM_BUILDER_DIST_FINAL) $(FRONTEND_FORM_EMBED_DIST_FINAL)/lm-forms.js $(FRONTEND_DIST)

.PHONY: build-email-builder
build-email-builder: $(FRONTEND_EMAIL_BUILDER_DIST_FINAL)

.PHONY: build-form-builder
build-form-builder: $(FRONTEND_FORM_BUILDER_DIST_FINAL)

.PHONY: build-form-embed
build-form-embed: $(FRONTEND_FORM_EMBED_DIST_FINAL)/lm-forms.js
